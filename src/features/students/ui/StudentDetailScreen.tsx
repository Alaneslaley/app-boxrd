...
export function StudentDetailScreen({ studentId }: { studentId: string }) {
  ...
  const canReadAttendance = state.status === 'authenticated' && can(state.permissions, 'ASISTENCIAS_CONSULTAR');
  const canRegisterAttendance = state.status === 'authenticated' && can(state.permissions, 'ASISTENCIAS_REGISTRAR');
